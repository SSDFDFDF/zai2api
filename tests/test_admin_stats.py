...
from app.services import token_stats_writer as token_stats_writer_module
from app.services.token_stats_writer import AsyncTokenStatsWriter
...
@pytest.mark.asyncio
async def test_token_pool_usage_stats_are_flushed_by_async_writer(tmp_path, monkeypatch):
    dao = TokenDAO(str(tmp_path / "token_usage_async.db"))
    await dao.init_database()
    token_id = await dao.add_token("zai", "token-usage-async", validate=False)
    assert token_id is not None

    pool = TokenPool([(token_id, "token-usage-async", "user")])
    writer = AsyncTokenStatsWriter(flush_interval=60.0)

    monkeypatch.setattr(token_pool_module, "_token_pool", pool)
    monkeypatch.setattr(token_dao_module, "_token_dao", dao)
    monkeypatch.setattr(token_stats_writer_module, "_token_stats_writer", writer)

    await pool.record_token_success("token-usage-async")
    await pool.record_token_success("token-usage-async")
    await pool.record_token_failure("token-usage-async", Exception("boom"))

    stats_before_flush = await dao.get_token_stats(token_id)
    assert stats_before_flush is not None
    assert stats_before_flush["total_requests"] == 0
    assert stats_before_flush["successful_requests"] == 0
    assert stats_before_flush["failed_requests"] == 0

    await writer.flush_now()

    stats_after_flush = await dao.get_token_stats(token_id)
    assert stats_after_flush is not None
    assert stats_after_flush["total_requests"] == 3
    assert stats_after_flush["successful_requests"] == 2
    assert stats_after_flush["failed_requests"] == 1

    await dao.close()


@pytest.mark.asyncio
async def test_sync_token_stats_to_db_flushes_async_queue_without_double_counting(
    tmp_path,
    monkeypatch,
):
    dao = TokenDAO(str(tmp_path / "token_usage_sync_queue.db"))
    await dao.init_database()
    token_id = await dao.add_token("zai", "token-usage-sync-queue", validate=False)
    assert token_id is not None

    pool = TokenPool([(token_id, "token-usage-sync-queue", "user")])
    writer = AsyncTokenStatsWriter(flush_interval=60.0)
    ...
    await pool.record_token_success("token-usage-sync-queue")
    await pool.record_token_failure("token-usage-sync-queue", Exception("boom"))
