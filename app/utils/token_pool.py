...
class TokenStatus:
    ...
    def is_healthy(self) -> bool:
        ...
        # 新 token 容错：请求数很少时，只要没触发连续错误熔断就健康
...
class TokenPool:
    ...
    def __init__(
        ...
        for token_id, token_value, token_type in tokens:
            if token_value and token_value not in self.token_statuses:
                ...
                if token_type == "user":
                    self._user_tokens.append(token_value)
    ...
    def _is_token_available(self, status: TokenStatus, current_time: float) -> bool:
        """检查 token 当前是否可用于轮询，必要时自动从熔断状态恢复。"""
        if status.token_type != "user":
            return False
        ...
        if status.cooldown_until and current_time >= status.cooldown_until:
            ...
            logger.info("✅ Token 熔断冷却结束，恢复可用: %s...", status.token[:20])
            return True
        ...
        async with self._lock:
            if token in self.token_statuses:
                ...
                if status.failure_count >= self.failure_threshold:
                    ...
                    status.cooldown_until = current_time + self.recovery_timeout
                    logger.warning(
                        "Token 已熔断: %s... (连续失败 %s 次, 冷却 %ss)",
                        token[:20],
    ...
    async def record_token_success(self, token: str, dao=None):
        ...
        if dao is None:
            ...
            writer = get_token_stats_writer()
            enqueued = writer.enqueue(
                token=token,
                token_id=token_id,
        ...
        *,
        ...
        async with self._lock:
            if token not in self.token_statuses:
                return
            status = self.token_statuses[token]
            status.db_synced_successful_requests += max(0, successful_requests)
            status.db_synced_failed_requests += max(0, failed_requests)
    ...
    async def get_pool_status(self) -> Dict:
        ...
        async with self._lock:
            ...
            available_count = sum(
                ...
                if (status := self.token_statuses.get(token))
                and self._is_token_available(status, current_time)
            )
        ...
        async with self._lock:
            if token in self.token_statuses:
                ...
                if old_type != token_type:
                    logger.debug("更新 Token 类型: %s... %s -> %s", token[:20], old_type, token_type)
    ...
    async def health_check_all(self):
        """异步健康检查所有 Token"""
        if not self.token_statuses:
            logger.debug("⚠️ Token 池为空，跳过健康检查")
            return
    ...
    async def sync_from_database(self, provider: str = "zai"):
        ...
        async with self._lock:
            ...
            if self._user_tokens:
                self._current_index %= len(self._user_tokens)
            else:
                self._current_index = 0
...
async def initialize_token_pool_from_db(
    ...
    if token_records:
        ...
        if len(user_tokens) < len(tokens):
            ...
            logger.debug("过滤了 %s 个匿名用户 Token", guest_count)
    ...
    async with _pool_lock:
        ...
        if not tokens:
            ...
            logger.debug("从数据库初始化 Token 池（%s），共 %s 个 Token", provider, len(tokens))
...
async def sync_token_stats_to_db():
    ...
    await get_token_stats_writer().flush_now()
