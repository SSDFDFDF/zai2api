...
class ModelManager:
    ...
    def __init__(self) -> None:
        ...
        self._casefold_index: Dict[str, str] = {}
    ...
    def apply_aliases(self, aliases_str: Optional[str] = None) -> None:
        ...
        if applied:
            logger.debug("Applied %d model alias(es)", applied)
    ...
    def _rebuild_casefold_index(self) -> None:
        """Rebuild case-insensitive lookup index from model_mapping keys."""
        self._casefold_index = {k.casefold(): k for k in self.model_mapping}

    def _resolve_name(self, model_name: str) -> Optional[str]:
        """Resolve model_name to its canonical key (case-insensitive).

        Tries exact match first, then falls back to case-insensitive lookup.
        """
        if model_name in self.model_mapping:
            return model_name
        return self._casefold_index.get(model_name.casefold())
    ...
    def get_upstream_model_id(self, model_name: str) -> Optional[str]:
        ...
        canonical = self._resolve_name(model_name)
        return self.model_mapping[canonical] if canonical else None

    def get_mcp_servers(self, model_name: str) -> List[str]:
        """Return the default MCP server list for *model_name*."""
        return self.model_mcp_servers.get(
            self._resolve_name(model_name) or model_name, []
        )

    def get_scene_defaults(self, model_name: str) -> Dict[str, Any]:
        """Return scene default parameters for *model_name*."""
        return self.model_scene_defaults.get(
            self._resolve_name(model_name) or model_name, {}
        )
