...
class SessionManager:
    ...
    async def create_session(
        ...
        logger.debug("新建会话: chat_id=%s... model=%s", chat_id[:8], model)
        return SessionResult(
            chat_id=chat_id,
            message_id=message_id,
            parent_id=None,
    ...
    async def commit_session_turn(
        ...
        logger.debug(
            "连续会话已提交: chat_id=%s last_message_id=%s",
            chat_id[:8],
            message_id[:8],
        )
    ...
    async def _update_fp_index(self, client_fp: str, chat_id: str) -> None:
        ...
        while len(chat_ids) > self._max_per_client:
            ...
            logger.debug("驱逐超限会话: %s...", evict_id[:8])
    ...
    async def _cleanup_loop(self) -> None:
        ...
        while True:
            try:
                ...
                if removed:
                    logger.debug("会话清理：清除 %s 条过期记录", removed)
            except asyncio.CancelledError:
                break
