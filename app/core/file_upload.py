...
async def upload_file(
    ...
    try:
        ...
        del encoded  # Release base64 string memory early
        ...
        logger.debug(
            "上传%s: %s, 大小: %s bytes", media_type, filename, len(file_data)
        )
        ...
        headers = {
            ...
            "Origin": base_url,
            "Referer": (
                f"{base_url}/c/{chat_id}" if chat_id else f"{base_url}/"
            ),
        ...
        logger.debug("[file] upload success: %s (%s)", file_id, file_name)
