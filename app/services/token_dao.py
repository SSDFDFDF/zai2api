...
class TokenDAO:
        ...
        try:
            ...
            if provider == "zai" and validate:
                ...
                if validated_type == "guest":
                    logger.warning("拒绝添加匿名用户 Token: %s... - %s", token[:20], error_msg)
                    return None

                # 拒绝无效 token
                if not is_valid:
                    logger.warning("Token 验证失败: %s... - %s", token[:20], error_msg)
                    return None
            ...
            async with self.session_factory() as session:
                ...
                if existing:
                    logger.debug("Token 已存在: %s - %s...", provider, token[:20])
                    return None
    ...
    async def record_stats_batch(self, updates: Dict[int, Dict[str, Any]]) -> None:
        ...
        try:
            async with self.session_factory() as session:
                for token_id, values in updates.items():
                    success_delta = int(values.get("successful_requests", 0) or 0)
                    failure_delta = int(values.get("failed_requests", 0) or 0)
                    if success_delta <= 0 and failure_delta <= 0:
                        continue

                    stmt_values: Dict[str, Any] = {
                        "total_requests": TokenStats.total_requests + success_delta + failure_delta,
                    }
                    if success_delta > 0:
                        stmt_values["successful_requests"] = (
                            TokenStats.successful_requests + success_delta
                        )
                        stmt_values["last_success_time"] = func.now()
                    if failure_delta > 0:
                        stmt_values["failed_requests"] = (
                            TokenStats.failed_requests + failure_delta
                        )
                        stmt_values["last_failure_time"] = func.now()

                    stmt = (
                        update(TokenStats)
                        .where(TokenStats.token_id == token_id)
                        .values(**stmt_values)
                    )
                    await session.execute(stmt)
                await session.commit()
        except Exception as e:
            logger.error(f"❌ 批量记录 Token 统计失败: {e}")
            raise
