            ...
            try:
                if settings.TOKEN_AUTO_IMPORT_ENABLED:
                    ...
                    if not source_dir:
                        ...
                        logger.debug(
                            "自动导入完成: scanned=%s imported=%s duplicate=%s invalid=%s",
            ...
            try:
                if settings.TOKEN_AUTO_MAINTENANCE_ENABLED:
                    ...
                    if not self._has_enabled_maintenance_action():
                        ...
                        logger.debug(
                            "自动维护完成: dedupe=%s checked=%s valid=%s guest=%s invalid=%s deleted=%s",
            ...
            try:
                if getattr(settings, "CHAT_CLEANUP_ENABLED", True):
                    ...
                    if summary.total_checked > 0:
                        logger.debug(
                            "定期会话清理完成: checked=%s success=%s failed=%s",
    ...
    async def _auto_log_cleanup_loop(self) -> None:
        while not self._stop_event.is_set():
            ...
            try:
                ...
                logger.debug(
                    "定期请求日志清理完成: retention_days=%s deleted=%s",
                    retention_days,
                    deleted_count,
                )
            except asyncio.CancelledError:
